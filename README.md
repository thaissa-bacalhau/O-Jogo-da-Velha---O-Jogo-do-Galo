...
O jogo possui um princípio de inteligencia artifical com as jogadas aleatórias da máquina que foi criado
...
void jogadaJogador();
void jogadaMaquina();
void limparBuffer(); //criei porque queria testar com o scanf (está presente em alguns subprogramas) || não utilizei o getch/getche
...
void jogarContraMaquina() {
    ...
    while (!fimDeJogo) { // em C ! significa negação (enquanto o jogo não terminar, é pra continuar a rodar, é tipo um while)
    	system("cls"); // limpar a tela e não mostrar as jogadas anteriores (eu usei isso para limpar a tela e não ficar mostrando as jogadas da anteriores da máquina)
...
void jogadaMaquina() {
    ...
    printf("Máquina jogou na posição: %d, %d\n", linha + 1, coluna + 1); // deixei essa função linha aqui, mas não será mostrado no executar por contar do system('cls')
